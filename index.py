...
class travel:
    ...
    #==========================================Define Functiom==================================================
        ...
        def Reset():
            ...
            self.txtBIKE .configure(state=DISABLED)
            self.txtAUTO.configure(state=DISABLED)
            self.txtAULTO.configure(state=DISABLED)
        ...
        def Receiptt():
            if reset_counter == 0 and Firstname.get()!="" and Surname.get()!="" and Address.get()!="" and Postcode.get()!="" and Mobile.get()!="" and Telephone.get()!="" and Email.get()!="":
                ...
                self.txtReceipt2.insert(END, BIKE .get() + "\n")
                self.txtReceipt1.insert(END,'Prime Sedan:\n')
                self.txtReceipt2.insert(END, AUTO.get() + "\n")
                self.txtReceipt1.insert(END,'Premium Sedan:\n')
                self.txtReceipt2.insert(END, AULTO.get() + "\n")
        ...
        def selectCar():
            ...
            if carType.get() == 1:
                ...
                self.txtAULTO.configure(state = DISABLED)
                AULTO.set("0")
                self.txtBIKE.configure(state = NORMAL)
                Item5 = float(8)
                BIKE.set("Rs "+ str(Item5))
            elif carType.get() == 2:
                self.txtBIKE.configure(state =DISABLED)
                BIKE.set("0")
                self.txtAULTO.configure(state = DISABLED)
                AULTO.set("0")
                self.txtAUTO.configure(state = NORMAL)
                Item5 = float(15)
                AUTO.set("Rs "+ str(Item5))
            else:
                self.txtBIKE.configure(state =DISABLED)
                BIKE.set("0")
                self.txtAUTO.configure(state = DISABLED)
                AUTO.set("0")
                self.txtAULTO.configure(state = NORMAL)
                Item5 = float(22)
                AULTO.set("Rs "+ str(Item5))
   ...
   #================================================mainframe========================================================================
    ...
    #==========================================================Cabselect=======================================================================
        ...
        self.chkBIKE =Radiobutton(Book_Frame,text="BIKE Cab",value=1,variable = carType,font=('arial',14,'bold'),command=selectCar).grid(row=0, column=0, sticky=W)
        self.txtBIKE= Label(Book_Frame,font=('arial',14,'bold'),width =7,textvariable=Standard,bd=5, state= DISABLED, justify=RIGHT,bg="white",relief=SUNKEN)
        self.txtBIKE.grid(row=0,column=1)
        

        self.chKAUTO=Radiobutton(Book_Frame,text="AUTO Cab",value=2,variable = carType,font=('arial',14,'bold'),command=selectCar).grid(row=1, column=0, sticky=W)
        self.txtAUTO= Label(Book_Frame,font=('arial',14,'bold'),width =7,textvariable=FordGalaxy,bd=5, state= DISABLED, justify=RIGHT,bg="white",relief=SUNKEN)
        self.txtAUTO.grid(row=1,column=1)
             
       
        self.chkAULTO = Radiobutton(Book_Frame,text="AULTO Cab",value=3,variable = carType,font=('arial',14,'bold'),command=selectCar).grid(row=2, column=0)
        self.txtAULTO = Label(Book_Frame,font=('arial',14,'bold'),width =7,textvariable=FordMondeo,bd=5, state= DISABLED, justify=RIGHT,bg="white",relief=SUNKEN)
        self.txtAULTO.grid(row=2,column=1)
